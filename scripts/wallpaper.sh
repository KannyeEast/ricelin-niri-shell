...
# niri reports outputs as a map keyed by connector name. A disconnected or
# disabled output is still in the map but has a null `logical`, and papering one
# of those would just write a file nothing ever reads, so they are filtered out.
outputs() {
    niri msg --json outputs 2>/dev/null \
        | jq -r 'to_entries | map(select(.value.logical != null)) | .[].key'
}
...
# niri's IPC exposes output geometry but not the pointer, so the monitor under
# the cursor cannot be resolved the way it was on Hyprland. The focused output
# is the honest approximation: it is the same monitor except in the moment
# between moving the mouse to another screen and clicking on it.
cursor_output() {
    focused_output
}
...
palette_update() {
    ...
    # No compositor reload here. Hyprland had to be told to re-read the colors
    # this just wrote; niri watches its own config and reloads on change.
